...
class FormationSpider(scrapy.Spider):
    ...
    #     super(FormationSpider, self).__init__(*args, **kwargs)
    #     self.i = 0
    ...
    def parse_formation(self, response):
        ...
        item['formation_rncp'] = response.xpath(".//a[contains(@href,'https://www.francecompetences.fr/recherche/rncp')]/text()").get()
        item['formation_rs'] = response.xpath(".//a[contains(@href,'https://www.francecompetences.fr/recherche/rs')]/text()").get()
        item['formation_reussite'] = response.xpath(".//b[contains(text(),'%')][1]/text()").get()
        
        print(f"intitulé: {item['formation_intitule']}")
        print(f"rncp: {item['formation_rncp']}")
        print(f"rs: {item['formation_rs']}")
        print(f"reussite: {item['formation_reussite']}")
    ...
    def parse_sessions_formation(self, response):
        ...
        if not blocs_sessions:
            ...
            # on ne yield pas pour pouvoir continuer de nourrir la variable dans la fonction suivante
                ...
                session_distanciel = None
                session_distanciel = True if (sess.xpath(".//div/a[contains(@href,'distanciel')]").get()) else False
                session_alternance = True if (sess.xpath(".//div/a[contains(@href,'https://simplon.co/i-apply?tags=alternance')]").get()) else False
                ...
                item['session_sous_intitule'] = sess.xpath(".//h2[@class='card-title']/text()").get()
                item['session_distanciel'] = session_distanciel
                item['session_alternance'] = session_alternance
                item['session_date_limite'] = session_date_limite if session_date_limite else None
                item['session_date_debut'] = sess.xpath(".//div[@class= 'card-session-info calendar']/text()").getall()
                item['session_duree'] = sess.xpath(".//div[@class= 'card-session-info'][1]/text()").getall()
                item['session_lieu'] = sess.xpath(".//div[contains(@class, 'card-content-tag-container')]/parent::*/text()").getall()
                item['session_region'] = sess.xpath(".//div[@class= 'card-session-info'][2]/text()").getall()
                item['session_niveau'] = sess.xpath(".//div[@class= 'card-session-info'][3]/text()").getall()
                yield item
                

                # Imprimer pour déboguer
                print(f"distanciel: {item['session_sous_intitule']}")
                print(f"distanciel: {item['session_distanciel']}")
                print(f"alt: {item['session_alternance']}")
                print(f"date limite: {item['session_date_limite']}")
                print(f"Date début: {item['session_date_debut']}")
                print(f"Durée: {item['session_duree']}")
                print(f"Lieu: {item['session_lieu']}")
                print(f"Niveau: {item['session_region']}")
                print(f"Niveau: {item['session_niveau']}")